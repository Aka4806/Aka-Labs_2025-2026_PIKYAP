...
def main():
    ...
    # Группируем по компьютерам
    res_1 = {}
    for browser_name, memory, comp_model in comps_with_a:
        if comp_model not in res_1:
            res_1[comp_model] = []
        # Добавляем браузер, если его еще нет в списке для этого компьютера
    ...
    # Сортировка по максимальному использованию памяти (по убыванию)
    ...
    # Сортируем по названию компьютера
    res_3 = sorted(many_to_many, key=itemgetter(2))

    # Группируем по компьютерам для вывода
    grouped_result = {}
